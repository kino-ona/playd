window.addEventListener("load", () => {
  ...
  navLinks.forEach((item) => {
    let itemIdx = parseInt(
      1 +
        Array.from(item.parentElement.querySelectorAll(".nav__link")).indexOf(
          item
        )
    );
    let targetNavmenuList = navmenu.querySelector(
      ".navmenu__list:nth-of-type(" + itemIdx + ")"
    );
    let targetNavLink = nav.querySelector(
      ".nav__link:nth-of-type(" + itemIdx + ")"
    );

    const navmenuListPos = () => {
      for (let i = 0; i < navmenuList.length; i++) {
        if (i === itemIdx - 1) {
          let elemWdL = navLinks[i]
            .querySelector("a")
